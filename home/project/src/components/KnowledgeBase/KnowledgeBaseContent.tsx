...
const features = {
  knowledgeSources: {
    ...
    description: 'Train your AI with your documentation, websites, and databases to provide accurate, contextual responses.',
    benefits: ['Automatic knowledge extraction', 'Real-time updates', 'Contextual understanding'],
  },
  stockPhrases: {
    title: 'Stock Phrases',
    description: 'Define exact responses for common questions, ensuring consistency and accuracy in AI responses.',
  ...
  scenarios: {
    ...
    benefits: ['Automated routing', 'Smart categorization', 'Priority handling'],
  },
  fields: {
    title: 'Fields',
    description: 'Let AI naturally collect important information during conversations without interrupting flow.',
    benefits: ['Natural data collection', 'Flexible requirements', 'Integration ready'],
  },
  botPersona: {
    title: 'Bot Persona',
    description: 'Customize how your AI presents itself to match your brand voice and style.',
    benefits: ['Brand consistency', 'Customizable personality', 'Professional appearance'],
  },
};
...
export function KnowledgeBaseContent({ isLoading }: KnowledgeBaseContentProps) {
  ...
  const [knowledgeSources, setKnowledgeSources] = useState(variantData.knowledgeSources);
  ...
  const [currentFeature, setCurrentFeature] = useState<keyof typeof features | null>(null);

  const isClassicOlark = currentPreset === 'Classic Olark';
  const showBotPersona = currentPreset !== 'Bespoke without Aiden' && !isClassicOlark;
  ...
  const handleSaveStockAnswer = (data: any) => {
    if (editingAnswer) {
      ...
      setStockAnswers((prev) => [...prev, { id: crypto.randomUUID(), ...data }]);
    }
        ...
        >
          <Stack direction="row" spacing={2} alignItems="center">
            ...
            <Typography variant="h5">
              Upgrade to AI-Powered Support
            </Typography>
          </Stack>
          <Typography variant="body1" sx={{ maxWidth: 600 }}>
            Transform your customer support with AI capabilities. Get instant responses,
            intelligent routing, and automated information collection - all while maintaining
            the personal touch your customers love.
          </Typography>
          <Button
            variant="contained"
            color="inherit"
            ...
            <Grid item xs={12} md={6} key={key}>
              <Box>
                <Box sx={{ display: 'flex', alignItems: 'center', gap: 1, mb: 2 }}>
                  ...
                  <IconButton size="small" onClick={() => handleHelpClick(key as keyof typeof features)}>
                    <HelpCircle size={20} />
                  </IconButton>
                </Box>
                <EmptyFeatureCard
